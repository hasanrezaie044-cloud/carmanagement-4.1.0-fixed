...
interface ServiceDao {
    ...
    // مجموع کیلومتر سرویس‌های ثبت‌شده از یک تاریخ به بعد (برای برآورد کیلومتر
    // فعلی از روی آخرین تعویض روغن، به‌جای تکیه بر بزرگ‌ترین کیلومتر تکی).
    @Query("SELECT COALESCE(SUM(km), 0) FROM services WHERE date > :afterDateExclusive")
    suspend fun sumKmAfter(afterDateExclusive: String): Double
