...
object Analytics {
    ...
    /**
     * Oil-change due status. Unlike [timingBeltDue] (still interval + last-maintenance-km
     * based), this now uses an explicit "current km / next km" target the user sets
     * directly (see AppSettings.oilChangeCurrentKm/oilChangeNextKm), with the distance
     * driven since that was set inferred from the km sum of services logged after
     * oilChangeSetAt — not from the single largest km value ever logged, which was
     * never a meaningful proxy for "current odometer".
     */
    fun oilChangeDue(currentKm: Double, nextKm: Double, kmSinceSetAt: Double): ServiceDue {
        if (nextKm <= 0.0) {
            return ServiceDue("تعویض روغن", null, 0, currentKm, null, false, false)
        }
        val estimatedOdometer = currentKm + kmSinceSetAt
        val interval = (nextKm - currentKm)
        val remaining = nextKm - estimatedOdometer
        return ServiceDue(
            label = "تعویض روغن",
            lastKm = currentKm,
            intervalKm = interval.toInt(),
            currentKm = estimatedOdometer,
            remainingKm = remaining,
            isOverdue = remaining <= 0,
            isDueSoon = interval > 0 && remaining in 0.0..(interval * 0.1),
        )
    }
