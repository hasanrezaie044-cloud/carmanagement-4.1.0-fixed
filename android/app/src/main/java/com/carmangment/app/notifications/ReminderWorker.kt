...
class ReminderWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    ...
    override suspend fun doWork(): Result {
        ...
        try {
            ...
            if (settings.remindOilChange) oilChange(ctx, repo, settings)
    ...
    private suspend fun oilChange(ctx: Context, repo: AppRepository, settings: AppSettings) {
        val kmSince = if (settings.oilChangeSetAt.isNotBlank()) repo.kmSince(settings.oilChangeSetAt) else 0.0
        val due = Analytics.oilChangeDue(settings.oilChangeCurrentKm, settings.oilChangeNextKm, kmSince)
